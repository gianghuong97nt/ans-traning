...
<div class="{{$col}} popup" data-search="lm004" data-istable="{{ $istable or 0}}" data-nm = "client_nm" data-id = "client_cd"
	...
	<div class="input-group" >
		<input maxlength="8" type="tel" class="form-control numeric left-radius right-radius refer-search-1 client_cd text-right {{isset($class)?$class:''}}" {{(isset($disabled) && $disabled != '')?'readonly':''}} style="width: 80px;border-top-right-radius: 3px!important;;border-bottom-right-radius: 3px!important" value="{{ $key_01 or '' }}" id="{{ $id_01 or 'client_cd' }}">
		<span class="input-group-text" style="padding-right: 5px">-</span>
		<div class="input-group" style="width: 80px">
			<input maxlength="4" type="tel" class="form-control numeric left-radius right-radius refer-search-1 client_br_cd text-right {{isset($class)?$class:''}}" {{(isset($disabled) && $disabled != '')?'readonly':''}} style="width: 65px;" value="{{ $key_02 or '' }}" id="{{ $id_02 or 'client_br_cd' }}">
		...
		@else
		<span class="input-group-text client_nm m-w-popup-label" id="{{$display_id or ''}}" style="    padding-left: 25px; {{ isset($istable) ? 'display: none;' : '' }}">{{ $value or '' }}
		</span>
		@endif
	</div>
</div>
