...
Public Class Frm_L013_Ex5
    ...
    Public Function FNC_EXL_L013(
        ...
        Try
            ...
            End If
            '
            D_EXL_TML = ConfigurationManager.AppSettings("FIL_TEM_EXL") & "\l013_temp2.xlsx"
            ...
            For I As Integer = 0 To D_PROJECT_CNT - 1
                If I < D_PROJECT_CNT - 1 Then
                    NAME_SHT = D_DAT.Tables(1).Rows(I).Item("project_no").ToString + "_" + D_DAT.Tables(1).Rows(I).Item("project_nm")
                ...
                PRO_COUNT = result.Count
                D_EXL_SHT.Cells(3, 13) = PAGE.ToString + "/" + D_DAT.Tables(1).Rows(I).Item("page_size").ToString
                For J As Integer = 0 To PRO_COUNT - 1
                ...
                'Fill dong tong
                '
                D_EXL_SHT.Cells(5 + PRO_COUNT + D_ROW, 11).Formula = "=SUM(K" & 5 & ":K" & (PRO_COUNT + D_ROW + 4).ToString & ")"
                D_EXL_SHT.Cells(5 + PRO_COUNT + D_ROW, 12).Formula = "=SUM(L" & 5 & ":L" & (PRO_COUNT + D_ROW + 4).ToString & ")"
                D_EXL_SHT.Cells(5 + PRO_COUNT + D_ROW, 13).Formula = "=SUM(M" & 5 & ":M" & (PRO_COUNT + D_ROW + 4).ToString & ")"
                '
            Next
