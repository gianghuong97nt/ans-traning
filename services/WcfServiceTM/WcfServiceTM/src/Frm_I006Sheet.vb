...
Public Class Frm_I006Sheet


    Public Function FNC_EXL_I006_Sheet(
        ...
        Try
            ...
            End If
            '
            D_EXL_TML = ConfigurationManager.AppSettings("FIL_TEM_EXL") & "\i006Sheet.xlsx"
            D_File_Log = ConfigurationManager.AppSettings("FIL_LOG")
            '
            D_Name = "I006Sheet_" & System.Guid.NewGuid.ToString() & Format(Date.Now, "yyyyMMddHHmmss")
            '
            ...
            Dim D_Name_page As String = ""

            Dim D_ROW As Integer = 0 'giup xac dinh vi tri cua ban ghi trong file
            Dim D_Page As Double = 1 'so page
            Dim D_MAXPage As Double = 1 'so page cua 1 nhom
            Dim D_Row_Page As Integer = 33 'max ban ghi 1 trang
            'tim max so nhom
            ...
            For I As Integer = 0 To D_GROUP_DIV - 1
                ...
                D_MAXPage = 1
                'tim max page
                D_MAXPage = Math.Ceiling(D_GROUP_ROW / D_Row_Page)
                D_ROW = 0
                D_Name_page = D_DAT.Tables(1).Rows(I).Item("detail_type_div").ToString + "_" + D_DAT.Tables(1).Rows(I).Item("detail_type_div_name")
                D_EXL_BOK.Sheets(1).Copy(After:=D_EXL_BOK.Sheets(I + 1))
                D_EXL_BOK.Sheets(I + 2).Name() = D_Name_page
                D_EXL_SHT = D_EXL_BOK.Sheets(D_Name_page)
                ...
                For J As Integer = 0 To D_GROUP_ROW - 1
                    ...
                    If J = D_Row_Page Then 'j bang voi max ban ghi tren 1 trang
                        D_EXL_BOK.Sheets(1).Rows("1:5").Copy() ' copy dinh dang tu sheet 2
                        D_EXL_SHT.Rows(D_ROW + 6 + 33).PasteSpecial() 'dan vao sheet 1 - vi tri hien tai da sang trang nen ko can tach trang
                        D_Page = D_Page + 1 'tang so trang len 1
                        D_EXL_SHT.Cells(D_ROW + 6 + 34, 12) = D_Page.ToString + "/" + D_MAXPage.ToString 'in so trang
                        D_ROW = D_ROW + 4
                        D_Row_Page = D_Row_Page + 33 'dieu kien tach trang tiep theo
                    End If
                    '
                ...
                D_EXL_BOK.Sheets(1).Rows("6:6").Copy()
...
ACTIVE:
        ...
        End Try
EXIT_FUNCTION:
        FNC_EXL_I006_Sheet = "{" + """" + "status" + """" + ":" + D_status + "," + """" + "filename" + """" + ":" + """" + D_FullName + """" + "}"
        Exit Function
