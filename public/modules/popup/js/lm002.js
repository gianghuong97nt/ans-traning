...
var _obj = {
      'section_nm'          : {'type': 'text', 'attr': {'maxlength': '50'    , 'class': ''}}
};
...
function initEvents() {
    try {
        ...
        $(document).on('dblclick', 'tbody tr', function () {
            try {
                var section_cd = $(this).attr('section_cd');
                window.location.href = '/master/m002?section_cd='+section_cd;
            } catch (e) {
                alert('special characters' + e.message);
            }
        });
...
function search(page){
    try{
        ...
        $.ajax({
            type: 'POST',
            url: '/popup/search/lm002/getdata',
            dataType: 'html',
            loading: true,
            data: data,
    ...
    try {
        ...
        for(var cl in row_obj){
            if($.inArray(parents.find('.' + cl+":visible").prop("tagName"), ["INPUT", "SELECT"]) != -1) {
                parents.find('.' + cl).val(row_obj[cl]).trigger('lm002change');
            } else {
