...
class lm001Controller extends Controller
{
    protected $detailScreen = 'M001';
    /**
     * display search
     *
    ...
    public function getIndex(Request $request)
    {
        ...
        return view('popup::search.lm001', $data, $data_screen);
    }
    ...
    public function postSearch(Request $request)
    {
        if($request->ajax()) {
        $param = $request->all();
        $data  = Dao::call_stored_procedure('SPC_LM001_FND1', $param);
        return view('popup::search.searchlm001')
            ->with('data', $data)
